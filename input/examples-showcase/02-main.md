...
flowchart TD
    ...
    subgraph Inputs ["Входные параметры (ControlMsg)"]
        ParSig["Параметры Сигнала<br/>(dev, n_dfm, len)"]
        ParStrobe["Параметры Строба<br/>(id, rays, delays)"]
        ParAntenna["Конфигурация Антенны<br/>(mask, coords)"]
        ParInterf["Параметры Помех<br/>(power, coords)"]
    end

    %% Генерация
    subgraph Stage1 ["1. Генерация (Имитатор)"]
        GenSig["Генерация образа сигнала"]
        GenNoise["Генерация Шума"]
        Mix["Смеситель"]
    ...
    %% Обработка
    subgraph Stage2 ["2. Пространственно-Временная Обработка (ПЧВО/ППО)"]
        PFTP["ПЧВО (LFM сжатие)"]
        PSP["ППО (Фазирование)"]
        
        Mix --> PFTP
        PFTP --> PSP
        
        ParSig -- "drofc (ЛЧМ)" --> PFTP
        ParAntenna -- "coords/phase" --> PSP
        ParStrobe -- "rays" --> PSP
    end
    
    subgraph Stage3 ["3. Очистка и Допплер (Адаптация/ЧВО)"]
        AutoComp["Автокомпенсация (418-02)"]
        TFP["ЧВО (Допплер/ФКМ)"]
        
        PSP --> AutoComp
        AutoComp --> TFP
        
        ParInterf -- "active jammer coords" --> AutoComp
        ParSig -- "n_dfm (M-Seq)" --> TFP
        ParStrobe -- "speed filters" --> TFP
    end
    
    subgraph Stage4 ["4. Обнаружение (ПЛО)"]
        Detect["Пороговая обработка"]
        Result["UCOPO (Отметки)"]
...
| несколько строк.  |  0    |   1   |     i. Вложенный,                                                                                    |
|                   |       |       |     i. Нумерованный,                                                                                 |
